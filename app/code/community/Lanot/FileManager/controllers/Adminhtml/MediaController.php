	...
	extends Mage_Adminhtml_Cms_Wysiwyg_ImagesController
{
    ...
    public function deleteFilesAction()
    {
        try {
            ...
            foreach ($files as $file) {
                $realFilename = $this->_getRealFilename($file);
                $this->getStorage()->delete($realFilename);
                $fileManagerStore = Mage::getModel('lanot_filemanager/fileStorage')->getCollection()
                    ->addByFileAttributes(pathinfo($realFilename))->getFirstItem();

                if ($fileManagerStore) {
                    $fileManagerStore->delete();
                }
        ...
        try {
            ...
            if (is_file($file) || is_dir($file)) {
                if(!$this->getStorage()->rename($file, $name)) {
                    ...
                    $fileManagerStore = Mage::getModel('lanot_filemanager/fileStorage')->getCollection()
                        ->addByFileAttributes(pathinfo($file))->getFirstItem();

                    $fileManagerStore->setFilename($pathinfo['basename'])
                        ->setType($pathinfo['extension'])
        ...
        try {
            ...
            foreach ($_files as $file) {
                ...
                if ($cut) {
                    ...
                    $fileManagerStore = Mage::getModel('lanot_filemanager/fileStorage');
                    $fileManagerStore->setCreatedTime(time());
                }

                $fileManagerStore->setDirectory($newfolder)
    ...
    public function uploadAction()
    {
        ...
        $fileManagerStore->setFilename($result['name'])
