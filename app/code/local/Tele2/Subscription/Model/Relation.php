...
class Tele2_Subscription_Model_Relation extends Mage_Core_Model_Abstract
{
    ...
    public function getProductsBySubscriptionGroup($subscriptionType1Id, $subscriptionType2Id, $_categoryId = null)
    {
    ...
    /**
     * Retrieves count of products from different combination of type1/type2 subscription attribute
     *
     * @param string $subscriptionType1Id
     * @param string $subscriptionType2Id
     * @param null|int $_categoryId
    ...
    public function getCountProductsBySubscriptionGroup($subscriptionType1Id, $subscriptionType2Id, $_categoryId = null)
    {
        if (!$this->_productCollection) {                       
            $this->_productCollection = Mage::getModel('catalog/product')->getCollection()
                ->addAttributeToSelect(Mage::getSingleton('catalog/config')->getProductAttributes())
        ...
        $data = $this->_productCollection->getConnection()->fetchPairs($this->_productCollection->getProductCountSelect());
        if ($_categoryId && isset($data[$_categoryId])) {
            return $data[$_categoryId];
        } else {
            return $data;
        }
    }
